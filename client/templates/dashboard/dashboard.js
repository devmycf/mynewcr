Template.dashboard.helpers({
 ...
 getMyPendings: function(){
     var num = Template.bookings.__helpers.get('numPending').call();
     return num;
 },
...
//    console.log(theBooking);
 ...
 numFuturas: function(){
     now = new Date();
     return Bookings.find({fechareco: {'$gte': now}}).count();
 },

 lastBooking: function(){
    var myBooking = Bookings.findOne({}, {sort: {createdAt: -1, limit: 1}});
    if(myBooking != null){
        return myBooking.nombre;
    }
 },
 ...
 isCompleted: function(id){
     var theBooking = Bookings.findOne({_id: id});
        ...
        if(ultimaActualizacion.length > 0){
...
Template.dashboard.rendered = function() {
  ...
  // });
  // var myReco = moment($(".updateRecoTime input").html()).format("DD/MM/YYYY HH:MM");
  // $(".updateRecoTime input").val(myReco);
  // var myDevo = moment($(".updateDevoTime input").html()).format("DD/MM/YYYY HH:MM");
}
