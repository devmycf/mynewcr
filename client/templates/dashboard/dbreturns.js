Template.returnBookings.helpers({
  devosFromNow: function(){
    now = new Date();
    now.setHours(0,0,0,0);
    return Bookings.find({fechadevo: {'$gte': now}}, {sort: {fechadevo: 1}});
  },
 ...
 nextDevo: function(){
     now = new Date();
    var myBooking = Bookings.findOne({fechadevo: {'$gte': now}}, {sort: {fechadevo: 1, limit: 1}});
     return myBooking.nombre;
 },

 nextDevoCar: function(){
     now = new Date();
    var myBooking = Bookings.findOne({fechadevo: {'$gte': now}}, {sort: {fechadevo: 1, limit: 1}});
...
Template.returnBookings.events({
        ...
        "change .bookingCar": function(e){
            ...
            // console.log(newCar);
            // Meteor.call("pushCarBooking", currentId, newCar);
