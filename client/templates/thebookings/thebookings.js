Template.thebookings.helpers({
  bookings: function(){
    switch (this.type) {
      case "M":
                var start = moment(""+this.month+"-01-"+this.year+"", "MM-DD-YYYY").toDate();
                if(this.month == 12){
                  var end = moment("01-01-"+(parseInt(this.year)+1)+"", "MM-DD-YYYY").toDate();
                } else {
                  var nextMonth = moment(parseInt(this.month)+1, 'M').format('MM');
                  var end = moment(""+nextMonth.toString()+"-01-"+this.year+"", "MM-DD-YYYY").toDate();
                }

                return Bookings.find({createdAt: {'$gte': start, '$lt': end}},{sort:{fechareco: 1}})
        break;
      case "Q":
              var theQuarter = this.quarter;
              var mesIni = 3*theQuarter - 2;
              var mesFin = 3*theQuarter + 1;
              var start = moment(""+mesIni+"-01-"+this.year+"", "MM-DD-YYYY").toDate();
              if(theQuarter == 4){
                console.log("ultimo");
                var end = moment("01-01-"+(parseInt(this.year)+1)+"", "MM-DD-YYYY").toDate();
              } else {
                var end = moment(""+mesFin+"-01-"+this.year+"", "MM-DD-YYYY").toDate();
              }
              return Bookings.find({createdAt: {'$gte': start, '$lt': end}},{sort:{fechareco: 1}})
        break;
        case "Y":
                var start = moment("01-01-"+this.year+"", "MM-DD-YYYY").toDate();
                var end = moment("12-31-"+this.year+"", "MM-DD-YYYY").toDate();
                return Bookings.find({createdAt: {'$gte': start, '$lte': end}},{sort:{fechareco: 1}})
        break;
        case "U":
                var now = new Date();
                now.setHours(0,0,0,0);
                return Bookings.find({fechareco: {'$gte': now}}, {sort: {fechareco: 1}});
        break;
        case "P":
  ...
  getTitle: function(){
    switch (this.type) {
      case "M":
            return "Reservas de "+moment().month(parseInt(this.month)-1, 'MM').format("MMMM")+" de "+this.year+" ";
        break;
      case "Q":
            return "Reservas del "+this.quarter+"T de "+this.year+" ";
        break;
      case "Y":
            return "Reservas de "+this.year+" ";
      break;
      case "U":
...
Template.thebookings.events({
  ...
  "change .ente-comisionado": function(e){
      ...
      var comiDate = new Date();

      if(comiID == 0){
        // console.log("lo");
        $(e.target).closest(".col-comision").find(".cteComision").addClass("hide");
        Meteor.call("resetComisionPerson", currentBooking, comiDate);
      } else {
        $(e.target).closest(".col-comision").find(".cteComision").removeClass("hide");
        Meteor.call("assignComisionPerson", currentBooking, comiID, comiName, comiDate);
      }
  },
